...
Almost all kubernetes options have been dropped in favor of the freeform syntax.

Previously, a lot was done to pre-configure Kubernetes in nixpkgs so that it provides a deploy-ready cluster with minimal user configuration. This "feature" is still available after this release, however there is now a more clear distinction between enabling individual raw kubernetes components and opting in for smart-configuration of a full Kubernetes cluster.

`services.kubernetes.roles` has been renamed to `services.kubernetes.autoConfigure.roles` and
in order to get auto-configuration, one must set `services.kubernetes.autoConfigure.enable` _and_
define roles - i.e. setting roles alone doesn't magically enable anything anymore.

Most notably, this change means that a lot of default values for cli flags have been removed in nixpkgs. Hence, when not using autoConfigure explicitly, these configuration options will revert to their upstream defaults. See below.

Flags that no longer have nixpkgs defaults which differed from upstream behavior **(action required)**:

kube-apiserver:

--api-audiences (nixpkgs: "api,https://kubernetes.default.svc") (upstream: <unset>)
--authorization-mode (nixpkgs: RBAC,Node) (upstream: AlwaysAllow)
--enable-admission-plugins (nixpkgs: NamespaceLifecycle,LimitRanger,ServiceAccount,ResourceQuota,DefaultStorageClass,DefaultTolerationSeconds,NodeRestriction) (upstream: <unset>)
--etcd-servers (nixpkgs: "http://127.0.0.1:2379") (upstream: <unset>)
--runtime-config (nixpkgs: "authentication.k8s.io/v1beta1=true") (upstream: <unset>)
--service-account-issuer (nixpkgs: "https://kubernetes.default.svc") (upstream: <unset>)
--service-cluster-ip-range (nixpkgs: "10.0.0.0/24") (upstream: <unset>)

kube-controller-manager:

--allocate-node-cidrs (nixpkgs: true) (upstream: false)
--bind-address (nixpkgs: "127.0.0.1") (upstream: "0.0.0.0")
--cluster-cidr (nixpkgs: "10.1.0.0/16") (upstream: <unset>)
--secure-port (nixpgks default: 10252) (upstream: 10257)
--use-service-account-credentials (nixpkgs: true) (upstream: false)

kube-proxy:

--cluster-cidr (nixpkgs: "10.1.0.0/16") (upstream: <unset>)
--hostname-override (nixpkgs: `config.networking.hostName`) (upstream: <unset>)

kube-scheduler:

--bind-address (nixpkgs: "127.0.0.1") (upstream: "0.0.0.0")
--secure-port (nixpkgs: 10251) (upstream: 10259)

kubelet:

--authentication-token-webhook (nixpkgs: true) (upstream: false)
--authentication-token-webhook-cache-ttl (nixpkgs: "10s") (upstream: "2m0s")
--authorization-mode (nixpkgs: "Webhook") (upstream: "AlwaysAllow")
--cgroup-driver (nixpkgs: "systemd") (upstream: "cgroupfs")
--container-runtime-endpoint (nixpkgs: "unix:///run/containerd/containerd.sock") (upstream: <unset>)
--hairpin-mode (nixpkgs: hairpin-veth) (upstream: promiscuous-bridge)
--hostname-override (nixpkgs: `config.networking.fqdnOrHostName`) (upstream: <unset>)
--pod-infra-container-image (nixpkgs: "pause") (upstream: "registry.k8s.io/pause:3.9")
--root-dir (nixpkgs: "/var/lib/kubernetes") (upstream: "/var/lib/kubelet")
